...
mkdir -p /root/.local/share/code-server/User; printf '%s' '{"terminal.integrated.defaultProfile.linux":"bash","terminal.integrated.profiles.linux":{"bash":{"path":"/bin/bash"}},"terminal.integrated.gpuAcceleration":"auto","terminal.integrated.fontSize":11,"terminal.integrated.lineHeight":1.15,"terminal.integrated.letterSpacing":0,"terminal.integrated.cursorBlinking":false,"terminal.integrated.rescaleOverlappingGlyphs":false,"terminal.integrated.smoothScrolling":false,"terminal.integrated.scrollback":2000,"editor.fontSize":12.5,"editor.minimap.enabled":false,"workbench.startupEditor":"none","workbench.tips.enabled":false,"window.zoomLevel":0}' > /root/.local/share/code-server/User/settings.json
if [ -d /root/.vscode-standalone ] && [ ! -L /root/.vscode-standalone/lib/node ]; then ln -sf \$(which node || echo /usr/bin/node) /root/.vscode-standalone/lib/node 2>/dev/null; fi

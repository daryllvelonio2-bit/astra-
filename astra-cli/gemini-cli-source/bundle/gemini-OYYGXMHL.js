...
import {
  ...
  updateExtension,
  validateAuthMethod
} from "./chunk-ASZFEU6C.js";
import {
  exitCli,
  require_source
} from "./chunk-FYAMITQ2.js";
import {
  appEvents
...
import {
  ...
  saveModelChange,
  stripAnsi
} from "./chunk-GAROUUGQ.js";
import {
  RELAUNCH_EXIT_CODE
} from "./chunk-LQJHQ4BU.js";
import "./chunk-XG56CL6B.js";
import {
  cleanupCheckpoints,
  registerCleanup,
  registerSyncCleanup,
...
import {
  resolveActiveModes,
  extractCognitiveMode,
  getActiveModeInstructions
} from "./astraCognitiveModes.js";
...
var MCP_LIST_DEFAULT_TIMEOUT_MSEC = 5e3;
...
var log = (msg) => debugLogger.log(msg);
var logError = (msg) => debugLogger.error(msg);
...
async function downloadFile(url, destPath) {
  ...
  try {
    for (; ; ) {
      ...
      if (!writeOk) {
        await new Promise((resolve9) => fileStream.once("drain", resolve9));
      }
...
function spawnInherited(command2, args) {
  return new Promise((resolve9, reject) => {
    const child = nodeSpawn(command2, args, {
      stdio: "inherit"
    });
...
async function handleSetup(argv) {
  ...
  log("");
  log(import_chalk10.default.bold("Gemma Local Model Routing Setup"));
  log(import_chalk10.default.dim("\u2500".repeat(40)));
  log("");
  const platform2 = detectPlatform();
  if (!platform2) {
    logError(
      import_chalk10.default.red(`Unsupported platform: ${process.platform}-${process.arch}`)
    );
    logError(
  ...
  log(import_chalk10.default.dim(`  Platform: ${platform2.key} \u2192 ${platform2.binaryName}`));
  if (!argv.consent) {
    log("");
  ...
  const binaryPath = getBinaryPath(platform2.binaryName);
  const alreadyInstalled = isBinaryInstalled();
  if (alreadyInstalled && !force) {
    log("");
    log(import_chalk10.default.green("  \u2713 LiteRT-LM binary already installed at:"));
    log(import_chalk10.default.dim(`    ${binaryPath}`));
  } else {
    log("");
    log("  Downloading LiteRT-LM binary...");
    const downloadUrl = getBinaryDownloadUrl(platform2.binaryName);
    debugLogger.log(`Downloading from: ${downloadUrl}`);
    try {
      const binDir = path4.dirname(binaryPath);
      fs3.mkdirSync(binDir, { recursive: true });
      await downloadFile(downloadUrl, binaryPath);
      log(import_chalk10.default.green("  \u2713 Binary downloaded successfully"));
    } catch (error) {
      logError(
    ...
    const expectedHash = PLATFORM_BINARY_SHA256[platform2.binaryName];
    if (!expectedHash) {
      logError(
        import_chalk10.default.red(
          `  \u2717 No checksum is configured for ${platform2.binaryName}. Refusing to install the binary.`
        )
      );
      try {
        fs3.rmSync(binaryPath, { force: true });
      } catch {
      }
      return 1;
    }
    try {
      ...
      log(import_chalk10.default.green("  \u2713 Binary checksum verified"));
    } catch (error) {
      logError(
        import_chalk10.default.red(
    ...
    if (process.platform === "darwin") {
      try {
        ...
        log(import_chalk10.default.green("  \u2713 macOS quarantine attribute removed"));
      } catch {
      }
    }
  }
  if (!argv.skipModel) {
    ...
    if (modelAlreadyDownloaded && !force) {
      log("");
      log(import_chalk10.default.green(`  \u2713 Model ${GEMMA_MODEL_NAME} already downloaded`));
    } else {
      log("");
      log(`  Downloading model ${GEMMA_MODEL_NAME}...`);
      log(import_chalk10.default.dim("  You may be prompted to accept the Gemma Terms of Use."));
      log("");
      ...
      if (exitCode !== 0) {
        logError("");
        logError(
          import_chalk10.default.red(`  \u2717 Model download failed (exit code ${exitCode})`)
        );
        return 1;
      }
      log("");
      log(import_chalk10.default.green(`  \u2713 Model ${GEMMA_MODEL_NAME} downloaded`));
    }
  }
  log("");
  ...
  try {
    const settings = loadSettings(process.cwd());
    const existingUserGemma = settings.forScope("User" /* User */).settings.experimental?.gemmaModelRouter ?? {};
    autoStartServer = existingUserGemma.autoStartServer ?? true;
    const existingUserExperimental = settings.forScope("User" /* User */).settings.experimental ?? {};
    settings.setValue("User" /* User */, "experimental", {
      ...existingUserExperimental,
      gemmaModelRouter: {
        autoStartServer,
        ...existingUserGemma.binaryPath !== void 0 ? { binaryPath: existingUserGemma.binaryPath } : {}
      }
    });
    const existingWorkspaceGemma = settings.forScope("Workspace" /* Workspace */).settings.experimental?.gemmaModelRouter ?? {};
    const existingWorkspaceExperimental = settings.forScope("Workspace" /* Workspace */).settings.experimental ?? {};
    settings.setValue("Workspace" /* Workspace */, "experimental", {
    ...
    log(import_chalk10.default.green("  \u2713 Settings updated"));
    log(import_chalk10.default.dim("    User (~/.gemini/settings.json): autoStartServer"));
    log(
      import_chalk10.default.dim("    Workspace (.gemini/settings.json): enabled, classifier")
    );
  ...
  if (argv.start) {
    ...
    if (serverStarted) {
      log(import_chalk10.default.green(`  \u2713 Server started on port ${port}`));
    } else {
      log(
  ...
  log(import_chalk10.default.dim("\u2500".repeat(40)));
  if (routingActive) {
    log(import_chalk10.default.bold.green("  Setup complete! Local model routing is active."));
  } else if (settingsUpdated) {
    log(
      import_chalk10.default.bold.green("  Setup complete! Local model routing is configured.")
    );
  } else {
    log(
      import_chalk10.default.bold.yellow(
  ...
  log(
    "  Simple tasks (file reads, quick edits) route to " + import_chalk10.default.cyan("Flash") + " for speed."
  );
  log(
    "  Complex tasks (debugging, architecture) route to " + import_chalk10.default.cyan("Pro") + " for quality."
  );
  ...
  if (!settingsUpdated) {
    ...
    log(import_chalk10.default.yellow('  Note: Run "gemini gemma start" to start the server.'));
    if (autoStartServer) {
      log(
        import_chalk10.default.yellow(
  ...
  log("  Useful commands:");
  log(import_chalk10.default.dim("    gemini gemma status   Check routing status"));
  log(import_chalk10.default.dim("    gemini gemma start    Start the LiteRT server"));
  log(import_chalk10.default.dim("    gemini gemma stop     Stop the LiteRT server"));
  log(import_chalk10.default.dim("    /gemma               Check status inside a session"));
  log("");
...
async function stopServer(expectedPort) {
...
var stopCommand = {
  ...
  handler: async (argv) => {
    ...
    if (pid !== null && isProcessRunning(pid)) {
      ...
      if (result === "stopped") {
        debugLogger.log(import_chalk11.default.green("LiteRT server stopped."));
        await exitCli(0);
        ...
        debugLogger.error(import_chalk11.default.red("Failed to stop LiteRT server."));
        await exitCli(1);
      }
      return;
    }
...
async function readLastLines(filePath, count) {
  ...
  const fileHandle = await fs5.promises.open(filePath, fs5.constants.O_RDONLY);
  try {
...
function waitForChild(child) {
  return new Promise((resolve9, reject) => {
    ...
    child.once("close", (code) => resolve9(code ?? 1));
  });
}
async function runTail(logPath, lines, follow) {
  const tailArgs = follow ? ["-f", "-n", String(lines), logPath] : ["-n", String(lines), logPath];
...
var logsCommand = {
  ...
  builder: (yargs) => yargs.option("lines", {
    alias: "n",
    type: "number",
  ...
  handler: async (argv) => {
    ...
    try {
      await fs5.promises.access(logPath, fs5.constants.F_OK);
    } catch {
    ...
    const follow = argv.follow ?? lines === void 0;
...
var gemmaCommand = {
  ...
  builder: (yargs) => yargs.middleware((argv) => {
    initializeOutputListenersAndFlush();
    argv["isCommand"] = true;
  }).command(defer(setupCommand, "gemma")).command(defer(startCommand, "gemma")).command(defer(stopCommand, "gemma")).command(defer(statusCommand, "gemma")).command(defer(logsCommand, "gemma")).demandCommand(1, "You need at least one command before continuing.").version(false),
  handler: () => {
  }
};
...
async function loadSandboxConfig(settings, argv) {
  ...
  const image = process.env["GEMINI_SANDBOX_IMAGE"] ?? "us-docker.pkg.dev/gemini-code-dev/gemini-cli/sandbox:0.57.0" ?? customImage ?? packageJson?.config?.sandboxImageUri;
...
function resolvePath(p) {
  ...
  return path6.normalize(expandedPath);
}
...
async function parseArguments(settings) {
  ...
  yargsInstance.command(
    ...
    (yargsInstance2) => yargsInstance2.positional("query", {
      ...
      type: "boolean",
      description: "Activate Fast Mode (instant concise generation)."
    }).option("medium", {
      type: "boolean",
      description: "Activate Medium Mode (balanced everyday engineering)."
    }).option("slow", {
      type: "boolean",
      description: "Activate Slow Mode (deep reasoning & unit tests)."
    }).option("superdeep", {
      type: "boolean",
      description: "Activate 10X Super Deep Mode with Kiro Spec-Driven Planning."
    }).option("spec", {
      type: "boolean",
      description: "Activate 10X Super Deep Mode with Kiro Spec-Driven Planning."
    }).option("godot", {
      type: "boolean",
      description: "Activate Godot 4.x Game Development Mode."
    }).option("godot-mobile", {
      type: "boolean",
      description: "Activate Godot 4.x Mobile Optimization Mode."
    }).option("godot-desktop", {
      type: "boolean",
      description: "Activate Godot 4.x Desktop Optimization Mode."
    })
...
async function loadCliConfig(settings, sessionId, argv, options = {}) {
  ...
  if (ideWorkspacePath) {
    ...
    const ideFolders = ideWorkspacePath.split(path7.delimiter).filter((p) => {
  ...
  return new Config({
    ...
    topicUpdateNarration: settings.general?.topicUpdateNarration ?? settings.experimental?.topicUpdateNarration,
...
async function resolveWorktreeSettings(cwd) {
  ...
  return {
    name: path7.basename(worktreePath),
    path: worktreePath,
...
function entrypoint(workdir, cliArgs) {
  ...
  if (fs6.existsSync(projectSandboxBashrc)) {
...
// packages/cli/src/utils/sandboxBuiltinProfiles.ts
var BUILTIN_SEATBELT_PROFILE_CONTENTS = {
...
BUILTIN_SEATBELT_PROFILE_CONTENTS["permissive-closed"] = BUILTIN_SEATBELT_PROFILE_CONTENTS["strict-open"];
BUILTIN_SEATBELT_PROFILE_CONTENTS["restrictive-closed"] = BUILTIN_SEATBELT_PROFILE_CONTENTS["strict-proxied"];
...
async function start_sandbox(config, nodeArgs = [], cliConfig, cliArgs = []) {
  ...
  patcher.patch();
  let stopProxy = void 0;
  let tempProfileFile = null;
  const cleanup = () => {
    if (tempProfileFile && fs7.existsSync(tempProfileFile)) {
      try {
        fs7.unlinkSync(tempProfileFile);
      } catch {
      }
  ...
  try {
    if (config.command === "sandbox-exec") {
      ...
      if (!BUILTIN_SEATBELT_PROFILES.includes(profile)) {
        const safeProfile = path8.basename(profile);
        const fileName = `sandbox-macos-${safeProfile}.sb`;
        const userProfileFile = path8.join(homedir(), GEMINI_DIR, fileName);
        const projectProfileFile = path8.join(GEMINI_DIR, fileName);
        profileFile = fs7.existsSync(userProfileFile) ? userProfileFile : projectProfileFile;
      } else {
        if (!fs7.existsSync(profileFile)) {
          const content = BUILTIN_SEATBELT_PROFILE_CONTENTS[profile];
          if (content) {
            try {
              const tempDir = fs7.realpathSync(os3.tmpdir());
              const rand = randomBytes(8).toString("hex");
              tempProfileFile = path8.join(
                tempDir,
                `gemini-sandbox-macos-${profile}-${rand}.sb`
              );
              fs7.writeFileSync(tempProfileFile, content, {
                encoding: "utf8",
                mode: 384
              });
      ...
      try {
        ...
        const args2 = [
          "-D",
          `TARGET_DIR=${fs7.realpathSync(process.cwd())}`,
          "-D",
          `TMP_DIR=${fs7.realpathSync(os3.tmpdir())}`,
          "-D",
          `HOME_DIR=${fs7.realpathSync(homedir())}`,
          "-D",
          `CACHE_DIR=${fs7.realpathSync((await execAsync("getconf DARWIN_USER_CACHE_DIR")).stdout.trim())}`
        ];
        const MAX_INCLUDE_DIRS = 5;
        const targetDir = fs7.realpathSync(cliConfig?.getTargetDir() || "");
        const includedDirs = [];
        if (cliConfig) {
        ...
        if (config.allowedPaths) {
          for (const hostPath of config.allowedPaths) {
            if (hostPath && path8.isAbsolute(hostPath) && fs7.existsSync(hostPath)) {
              const realDir = fs7.realpathSync(hostPath);
        ...
        args2.push(
          ...
          [
            ...
            "NODE_OPTIONS=" + (0, import_shell_quote2.quote)([nodeOptions]),
        ...
        if (proxyCommand2) {
          ...
          proxyProcess2 = spawn2(proxyCommand2, {
        ...
        process.stdin.pause();
        sandboxProcess2 = spawn2(config.command, args2, {
          stdio: "inherit"
        });
        return await new Promise((resolve9, reject) => {
          sandboxProcess2?.on("error", (err) => {
            cleanup();
            reject(err);
          });
    ...
    const gcPath = process.argv[1] ? fs7.realpathSync(process.argv[1]) : "";
    const projectSandboxDockerfile = path8.join(
      GEMINI_DIR,
      "sandbox.Dockerfile"
    );
    const isCustomProjectSandbox = fs7.existsSync(projectSandboxDockerfile);
    ...
    if (process.env["BUILD_SANDBOX"]) {
      if (!gcPath.includes("gemini-cli/packages/")) {
        ...
        let buildArgs = "";
        const projectSandboxDockerfile2 = path8.join(
          GEMINI_DIR,
          "sandbox.Dockerfile"
        );
        if (isCustomProjectSandbox) {
          debugLogger.log(`using ${projectSandboxDockerfile2} for sandbox`);
          buildArgs += `-f ${path8.resolve(projectSandboxDockerfile2)} -i ${image}`;
        }
        execSync(
    ...
    if (!fs7.existsSync(userHomeDirOnHost)) {
      fs7.mkdirSync(userHomeDirOnHost, { recursive: true });
    }
    const userSettingsDirOnHost = path8.join(userHomeDirOnHost, GEMINI_DIR);
    if (!fs7.existsSync(userSettingsDirOnHost)) {
      fs7.mkdirSync(userSettingsDirOnHost, { recursive: true });
    }
    args.push(
      "--volume",
    ...
    const gcloudConfigDir = path8.join(homedir(), ".config", "gcloud");
    if (fs7.existsSync(gcloudConfigDir)) {
      args.push(
        "--volume",
    ...
    if (process.env["VIRTUAL_ENV"]?.toLowerCase().startsWith(workdir.toLowerCase())) {
      const sandboxVenvPath = path8.resolve(GEMINI_DIR, "sandbox.venv");
      if (!fs7.existsSync(sandboxVenvPath)) {
        fs7.mkdirSync(sandboxVenvPath, { recursive: true });
      }
      args.push(
        "--volume",
    ...
    if (command2 === "podman") {
      const emptyAuthFilePath = path8.join(os3.tmpdir(), "empty_auth.json");
      fs7.writeFileSync(emptyAuthFilePath, "{}", "utf-8");
    ...
    if (proxyCommand) {
      ...
      proxyProcess = spawn2(command2, proxyContainerArgs, {
        stdio: ["ignore", "pipe", "pipe"],
        shell: false,
      ...
      stopProxy = () => {
        ...
        try {
          spawnSync(command2, ["rm", "-f", SANDBOX_PROXY_NAME], {
            stdio: "ignore"
          });
        } catch {
        }
    ...
    process.stdin.pause();
    sandboxProcess = spawn2(command2, args, {
      stdio: "inherit"
    });
    return await new Promise((resolve9, reject) => {
...
async function start_lxc_sandbox(config, nodeArgs = [], cliArgs = []) {
  ...
  try {
    ...
    process.stdin.pause();
    const sandboxProcess = spawn2("lxc", args, {
      stdio: "inherit"
    });
    return await new Promise((resolve9, reject) => {
      sandboxProcess.on("error", (err) => {
...
async function imageExists(sandbox, image) {
  return new Promise((resolve9) => {
    ...
    const checkProcess = spawn2(sandbox, args);
    let stdoutData = "";
...
async function pullImage(sandbox, image, cliConfig) {
  ...
  return new Promise((resolve9) => {
    ...
    const pullProcess = spawn2(sandbox, args, { stdio: "pipe" });
    let stderrData = "";
...
async function getStartupWarnings() {
  try {
    await fs8.access(warningsFilePath);
    const warningsContent = await fs8.readFile(warningsFilePath, "utf-8");
    const warnings = warningsContent.split("\n").filter((line) => line.trim() !== "");
    try {
      await fs8.unlink(warningsFilePath);
    } catch {
  ...
  check: async (workspaceRoot, settings) => {
    ...
    try {
      const [workspaceRealPath, homeRealPath] = await Promise.all([
        fs9.realpath(workspaceRoot),
        fs9.realpath(osHomedir())
      ]);
      if (path9.resolve(workspaceRealPath) === path9.resolve(homeRealPath)) {
...
var rootDirectoryCheck = {
  ...
  check: async (workspaceRoot, _settings) => {
    try {
      ...
      if (path9.dirname(workspaceRealPath) === workspaceRealPath) {
        return errorMessage;
      }
      return null;
    } catch {
...
async function runNonInteractive({
  ...
  return promptIdContext.run(prompt_id, async () => {
    ...
    if (process.env["GEMINI_CLI_ACTIVITY_LOG_TARGET"]) {
      const { setupInitialActivityLogger } = await import("./devtoolsService-WMEEEXY6.js");
...
`);
    ...
    try {
      ...
      const activeModes = resolveActiveModes(config.getArgv?.() || globalThis.__astraArgv || {}, input);
      const astraModeInstructions = getActiveModeInstructions(activeModes);
      if (astraModeInstructions) {
        globalThis.__astraActiveInstructions = astraModeInstructions;
      }
      let query;
...
`);
    ...
    const setupStdinCancellation = () => {
      ...
      readline3.emitKeypressEvents(process.stdin, rl);
      const keypressHandler = (str, key) => {
        if (key && key.ctrl && key.name === "c" || str === "") {
          if (isAborting) {
            return;
          ...
          if (streamFormatter) {
            ...
            textOutput.write(
              formatter.format(
                ...
                responseText,
                stats,
                invalidStreamError ? { type: "INVALID_STREAM", message: invalidStreamError } : void 0,
                warnings
              )
            );
          } else {
...
var InboxMemoryCommand = class {
  ...
  async execute(context, _) {
    ...
    for (const s of skills) {
      const date = s.extractedAt ? ` (extracted: ${new Date(s.extractedAt).toLocaleDateString()})` : "";
      lines.push(`- **${s.name}**: ${s.description}${date}`);
    }
    for (const p of patches) {
      const targets = p.entries.map((e) => e.targetPath).join(", ");
      const date = p.extractedAt ? ` (extracted: ${new Date(p.extractedAt).toLocaleDateString()})` : "";
    ...
    for (const memoryPatch of memoryPatches) {
      ...
      const date = memoryPatch.extractedAt ? ` (latest extract: ${new Date(memoryPatch.extractedAt).toLocaleDateString()})` : "";
...
var InitCommand = class {
  ...
  async execute(context, _args = []) {
    ...
    const result = performInit(fs10.existsSync(geminiMdPath));
    switch (result.type) {
      case "message":
        return {
          name: this.name,
      ...
      case "submit_prompt":
        fs10.writeFileSync(geminiMdPath, "", "utf8");
...
var RestoreCommand = class {
  ...
  async execute(context, args) {
    ...
    try {
      ...
      let data;
      try {
        data = await fs11.readFile(filePath, "utf-8");
      } catch (error) {
...
var ListCheckpointsCommand = class {
  ...
  async execute(context) {
    ...
    try {
      ...
      const checkpointDir = config.storage.getProjectTempCheckpointsDir();
      try {
        await fs11.mkdir(checkpointDir, { recursive: true });
      } catch {
      }
      const files = await fs11.readdir(checkpointDir);
...
// packages/cli/src/acp/acpSession.ts
var StructuredErrorSchema = external_exports.object({
...
${event.value.description}`;
            ...
            case GeminiEventType.Finished: {
              ...
              if (usage2) {
                ...
                turnOutputTokens = usage2.candidatesTokenCount ?? turnOutputTokens;
              }
              break;
            }
...
var AcpFileSystemService = class {
  constructor(connection, sessionId, capabilities, fallback, root) {
    this.connection = connection;
    this.sessionId = sessionId;
  ...
  geminiDir = path13.join(os5.homedir(), ".gemini");
...
var AcpSessionManager = class {
  constructor(settings, argv, connection) {
    this.settings = settings;
    this.argv = argv;
    this.connection = connection;
  }
  ...
  async newSession({ cwd, mcpServers }, authDetails) {
  ...
  async loadSession({ sessionId, cwd, mcpServers }, authDetails) {
...
async function setupTerminalAndTheme(config, settings) {
  ...
  if (terminalBackground !== void 0 && (settings.merged.ui.autoThemeSwitching ?? true)) {
...
async function cleanupBackgroundLogs(debugMode = false) {
  try {
    ...
    try {
      await fs13.access(logDir);
    } catch {
      return;
    }
    const entries = await fs13.readdir(logDir, { withFileTypes: true });
    const now = Date.now();
    let deletedCount = 0;
    for (const entry of entries) {
      if (entry.isFile() && entry.name.endsWith(".log")) {
        const filePath = path14.join(logDir, entry.name);
        try {
...
async function startInteractiveUI(config, settings, startupWarnings, workspaceRoot = process.cwd(), resumedSessionData, initializationResult) {
  const { startInteractiveUI: doStartUI } = await import("./interactiveCli-5O6FZS57.js");
  await doStartUI(
    config,
    settings,
    startupWarnings,
...
${finalArgs[promptIndex + 1]}`;
  ...
  }
  {
    ...
    initAppHandle?.end();
    import("./liteRtServerManager-LK6HOSMO.js").then(({ LiteRtServerManager }) => {

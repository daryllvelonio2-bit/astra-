...
import {
  ...
  updateExtension,
  validateAuthMethod
} from "./chunk-IKHAM2LG.js";
import {
  exitCli,
  require_source
} from "./chunk-HY6GN2F7.js";
import {
  appEvents
...
import {
  ...
  saveModelChange,
  stripAnsi
} from "./chunk-GPOBVDAD.js";
import {
  RELAUNCH_EXIT_CODE
} from "./chunk-ISXOUBN5.js";
import "./chunk-FVC4WCSP.js";
import {
  cleanupCheckpoints,
  registerCleanup,
  registerSyncCleanup,
...
async function loadSandboxConfig(settings, argv) {
  ...
  const image = process.env["GEMINI_SANDBOX_IMAGE"] ?? "us-docker.pkg.dev/gemini-code-dev/gemini-cli/sandbox:0.57.0-preview.1" ?? customImage ?? packageJson?.config?.sandboxImageUri;
...
async function runNonInteractive({
  ...
  return promptIdContext.run(prompt_id, async () => {
    ...
    if (process.env["GEMINI_CLI_ACTIVITY_LOG_TARGET"]) {
      const { setupInitialActivityLogger } = await import("./devtoolsService-7YYOUGVF.js");
...
async function startInteractiveUI(config, settings, startupWarnings, workspaceRoot = process.cwd(), resumedSessionData, initializationResult) {
  const { startInteractiveUI: doStartUI } = await import("./interactiveCli-5E2C3XOC.js");
  await doStartUI(
    config,
    settings,
    startupWarnings,
...
${finalArgs[promptIndex + 1]}`;
  ...
  }
  {
    ...
    initAppHandle?.end();
    import("./liteRtServerManager-5QBFCLBK.js").then(({ LiteRtServerManager }) => {
